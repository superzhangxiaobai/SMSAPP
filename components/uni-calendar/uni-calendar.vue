			...
			<view class="uni-calendar__wrapper">
				<view class="uni-calenda__content">
					<view class="uni-calendar__panel">
						...
						<view class="uni-calendar__signin" @tap="checkin">今日打卡</view>
						<view class="uni-calendar__backtoday" @tap="backtoday">回到今天</view>
					</view>
					<view v-if="lunar" class="uni-calendar__day-detail">
						<view>
							{{canlender.year+'年'+canlender.month+'月'+canlender.date+'日('+canlender.lunar.astro +')'}}
						</view>
						<view>
							{{canlender.lunar.gzYear +'年' +canlender.lunar.gzMonth +'月'+canlender.lunar.gzDay +'日 (' +
								canlender.lunar.Animal +'年)'}}
					...
					<swiper v-else :style="{ height: domHeihgt + 'px' }" :current="currentIndex" :vertical="slide === 'vertical' ? true : false" :duration="duration" class="uni-calendar__body" circular skip-hidden-item-layout @animationfinish="animationfinish" @change="change">
						<swiper-item v-for="(item, itemindx) in swiperData" :key="itemindx">
							<view :class="elClass">
								<uni-calendar-item :selectColor="selectColor" :canlender="item" :lunar="lunar" @selectDays="selectDays" />
							</view>
						</swiper-item>
					</swiper>
				</view>
...
<style>
	...
	.uni-calendar__wrapper .uni-calenda__content .uni-calendar__panel .uni-calendar__signin {
		position: absolute;
		right: 0;
		top: 15rpx;
		padding: 0 10rpx;
